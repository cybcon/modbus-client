...
def parse_modbus_result(registers: list, startRegister: int, readLength: int, valueType: str, big_endian: bool = False):
    ...
    for register in registers:
        ...
        if valueType == "word":
            ...
            if big_endian:
                htext32 = previousRegister32 + htext
            else:
                htext32 = htext + previousRegister32
...
group.add_argument(
    "-s", "--slave", help="Hostname or IP address of the Modbus TCP slave (default: 127.0.0.1)", default="127.0.0.1"
)
...
group.add_argument(
    "-t",
    "--registerType",
    help="Register type 1 to 4 to read (1=Discrete Output Coils, 2=Discrete Input Contacts, 3=Analog Output Holding Register, 4=Analog Input Register) (default: 3)",
    default=3,
)
group.add_argument("-r", "--register", help="The register address between 0 and 9999 (default: 0)", default=0)
group.add_argument(
    "-l", "--length", help="How many registers should be read, between 1 and 125 (default: 1)", default=1
)
group.add_argument(
    "-b",
    "--bigEndian",
    help="Use big endian instead of little endian when calculating the 32bit values",
    action="store_true",
    default=False,
)
...
if args.registerType >= 3:
    df = parse_modbus_result(
        registers=rr.registers,
        startRegister=register_number,
        readLength=args.length,
        valueType="word",
        big_endian=args.bigEndian,
    )
...
with pd.option_context(
    "display.max_rows", None, "display.max_columns", None, "display.float_format", lambda x: "%.6f" % x
):
    if args.csv:
